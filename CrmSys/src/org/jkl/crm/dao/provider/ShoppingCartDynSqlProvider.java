...
public class ShoppingCartDynSqlProvider {
	/**
	...
	public String selectWithParam(Map<String, Object> params) {
		String sql = new SQL() {
			{
				SELECT("*");
				FROM("(SELECT *,ROW_NUMBER() OVER(ORDER BY ID) AS RowId FROM " + CARTTABLE + ")AS T");
				if (params.get("cart") != null) {
					ShoppingCart cart = (ShoppingCart) params.get("cart");
	 ...
	 * @return
	 */
	public String updateCart(ShoppingCart cart) {//购物车判断更新count（商品数量）是否为0，如果为0，则调用delete
		String sql = new SQL() {
			{
				UPDATE(CARTTABLE);
				//如果是0,调用delete，前端判断
