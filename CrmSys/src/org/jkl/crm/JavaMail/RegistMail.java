 ...
 * @author Alan
 */
public class RegistMail {
	/**
	 * 发送操作
	 * @param user
	 */
	public static void sendRegistMail(User user) {
		...
		try {
			...
			// 创建邮件,并且发送给当前修改密码的用户
			MimeMessage message = createMimeMessage(ssession, "285692983@qq.com",user.getName(), user.getEmail());
	...
	public static MimeMessage createMimeMessage(Session session, String sender, 
		...
		try {
			...
			Address address = new InternetAddress(sender, "Dick", "utf-8");
			message.setSubject("注册通知邮件", "utf-8");//邮件主题
			 ...
			 */
			DataSource file = new FileDataSource("D:/bookStore/mail.jpg");
			...
			//邮件主体内容
			textPart.setContent("亲爱的"+username+"你好，您刚刚在-有座书城-网站完成了注册！如果是本人操作，请忽略此邮件信息。<br/><img src='cid:cool'/>", "text/html;charset=utf-8");
			...
			message.setRecipient(MimeMessage.RecipientType.TO, new InternetAddress(receiver, "你算哪块小饼干", "utf-8"));
