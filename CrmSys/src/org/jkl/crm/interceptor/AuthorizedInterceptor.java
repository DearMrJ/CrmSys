package org.jkl.crm.interceptor;
...
public class AuthorizedInterceptor implements HandlerInterceptor{
	/**定义不需要拦截的请求**/
	private static final String[] IGNORE_URI = {"/index","/loginForm","/login","404","/main"};
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		if(!flag) {
			/**	1.获取session中的用户 */
			User user = (User) request.getSession().getAttribute(CrmConstants.USER_SESSION);	
			/** 2.判断用户是否已经登录 */
			if(user == null) {
				/** 如果用户没有登录，跳转到登录页面 */
				request.setAttribute("message", "请先登录再进行访问！");
				/**没有登录则跳转至loginForm**/
				request.getRequestDispatcher(CrmConstants.LOGIN).forward(request, response);
				return flag;
			}else {
				flag=true;
			}
